...
_cors_origins_raw = os.getenv("CORS_ALLOW_ORIGINS", "https://hekeyi0386-dot.github.io")
...
def _parse_plan_json(raw: Optional[str]) -> dict:
    """从 AI 回复中提取 JSON，兼容前后有多余文字或空内容的情况。"""
...
def plan(req: PlanRequest):
    ...
    try:
        ...
        timeout = float(os.getenv("MODEL_TIMEOUT", "20"))
        ...
        if hf_token:
            ...
            try:
                ...
                raw_content = _safe_str(resp.choices[0].message.content)
            except Exception as e:
        ...
        return PlanResponse(
            priority_summary  = _safe_str(data.get("priority_summary", "")),
            today_plan        = _safe_list_str(data.get("today_plan", [])),
            build_priority    = _safe_str(data.get("build_priority", "")),
            sell_keep_process = _safe_list_str(data.get("sell_keep_process", [])),
            next_3_day_focus  = _safe_list_str(data.get("next_3_day_focus", [])),
            reason            = _safe_str(data.get("reason", "")),
            risk_warning      = _safe_str(data.get("risk_warning", "")),
            risk              = _safe_str(data.get("risk", "medium")) or "medium",
        )
    except Exception as e:
        # 最后一层兜底：即便出现未预期异常，也保证前端拿到可渲染数据。
        data = _build_local_fallback_plan(req, f"服务异常已兜底: {e}")
        return PlanResponse(**data)
